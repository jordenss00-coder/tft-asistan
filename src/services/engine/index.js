 ...
 * }
 */
function coachNow({ S, metaComps, stats, state }) {
  const recs = recommendComps({ S, metaComps, stats, state });
  ...
  const items = adviseItems({ S, stats, comp: chosen, components: state.components || [], completed: state.completed || [], stage: state.stage });
